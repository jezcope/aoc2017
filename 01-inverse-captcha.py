...
def inverse_captcha_(s, acc=0):
    _coconut_match_to = reiterable(s)
    _coconut_match_check = False
    if _coconut.isinstance(_coconut_match_to, _coconut.abc.Iterable):
        rest = _coconut.iter(_coconut_match_to)
        _coconut_match_temp_0 = _coconut.tuple(_coconut_igetitem(rest, _coconut.slice(None, 2)))
        if (_coconut.len(_coconut_match_temp_0) == 2) and (_coconut_match_temp_0[0] == _coconut_match_temp_0[1]):
            d = _coconut_match_temp_0[0]
            _coconut_match_check = True
    if _coconut_match_check:
        return _coconut_tail_call(inverse_captcha_, _coconut.itertools.chain.from_iterable((f() for f in (lambda: (f() for f in (lambda: d,)), lambda: rest))), acc + int(d))
    if not _coconut_match_check:
        if _coconut.isinstance(_coconut_match_to, _coconut.abc.Iterable):
            rest = _coconut.iter(_coconut_match_to)
            _coconut_match_temp_0 = _coconut.tuple(_coconut_igetitem(rest, _coconut.slice(None, 2)))
            if _coconut.len(_coconut_match_temp_0) == 2:
                d0 = _coconut_match_temp_0[0]
                d1 = _coconut_match_temp_0[1]
                _coconut_match_check = True
        if _coconut_match_check:
            return _coconut_tail_call(inverse_captcha_, _coconut.itertools.chain.from_iterable((f() for f in (lambda: (f() for f in (lambda: d1,)), lambda: rest))), acc)
...
def inverse_captcha(s):
    return _coconut_tail_call(inverse_captcha_, _coconut.itertools.chain.from_iterable((f() for f in (lambda: s, lambda: s[0]))))


@_coconut_tco
def inverse_captcha_1_(s0, s1, acc=0):
    def _coconut_mock_func(s0, s1, acc=0): return s0, s1, acc
    while True:
        _coconut_match_to = (reiterable(s0), reiterable(s1))
        _coconut_match_check = False
        if (_coconut.isinstance(_coconut_match_to, _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to) == 2) and (_coconut.isinstance(_coconut_match_to[0], _coconut.abc.Iterable)) and (_coconut.isinstance(_coconut_match_to[1], _coconut.abc.Iterable)):
            rest0 = _coconut.iter(_coconut_match_to[0])
            _coconut_match_temp_0 = _coconut.tuple(_coconut_igetitem(rest0, _coconut.slice(None, 1)))
            rest1 = _coconut.iter(_coconut_match_to[1])
            _coconut_match_temp_1 = _coconut.tuple(_coconut_igetitem(rest1, _coconut.slice(None, 1)))
            if (_coconut.len(_coconut_match_temp_0) == 1) and (_coconut.len(_coconut_match_temp_1) == 1) and (_coconut_match_temp_0[0] == _coconut_match_temp_1[0]):
                d0 = _coconut_match_temp_0[0]
                _coconut_match_check = True
        if _coconut_match_check:
            if inverse_captcha_1_ is _coconut_recursive_func_2:
                s0, s1, acc = _coconut_mock_func(rest0, rest1, acc + int(d0))
                continue
            else:
                return _coconut_tail_call(inverse_captcha_1_, rest0, rest1, acc + int(d0))
        if not _coconut_match_check:
            if (_coconut.isinstance(_coconut_match_to, _coconut.abc.Sequence)) and (_coconut.len(_coconut_match_to) == 2) and (_coconut.isinstance(_coconut_match_to[0], _coconut.abc.Iterable)) and (_coconut.isinstance(_coconut_match_to[1], _coconut.abc.Iterable)):
                rest0 = _coconut.iter(_coconut_match_to[0])
                _coconut_match_temp_0 = _coconut.tuple(_coconut_igetitem(rest0, _coconut.slice(None, 1)))
                rest1 = _coconut.iter(_coconut_match_to[1])
                _coconut_match_temp_1 = _coconut.tuple(_coconut_igetitem(rest1, _coconut.slice(None, 1)))
                if (_coconut.len(_coconut_match_temp_0) == 1) and (_coconut.len(_coconut_match_temp_1) == 1):
                    d0 = _coconut_match_temp_0[0]
                    d1 = _coconut_match_temp_1[0]
                    _coconut_match_check = True
            if _coconut_match_check:
                if inverse_captcha_1_ is _coconut_recursive_func_2:
                    s0, s1, acc = _coconut_mock_func(rest0, rest1, acc)
                    continue
                else:
                    return _coconut_tail_call(inverse_captcha_1_, rest0, rest1, acc)

        return acc


        return None
_coconut_recursive_func_2 = inverse_captcha_1_
@_coconut_tco
def inverse_captcha_1(s):
    return _coconut_tail_call(inverse_captcha_1_, s, _coconut.itertools.chain.from_iterable((f() for f in (lambda: _coconut_igetitem(s, _coconut.slice(len(s) // 2, None)), lambda: s))))
...
def test_inverse_captcha_1():
    assert (inverse_captcha_1)("1212") == 6
    assert (inverse_captcha_1)("1221") == 0
    assert (inverse_captcha_1)("123425") == 4
    assert (inverse_captcha_1)("123123") == 12
    assert (inverse_captcha_1)("12131415") == 4

if __name__ == "__main__":
    (print)((inverse_captcha)(sys.argv[1]))
    (print)((inverse_captcha_1)(sys.argv[1]))
